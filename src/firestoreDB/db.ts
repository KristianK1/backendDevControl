...
export class Db {
    ...
    async addUser(username: string, password: string, email: string): Promise<number> {
        ...
        var maxIDdoc = await this.getMaxUserId(true);
    ...
    async changeUserPassword(id: number, oldP: string, newP: string) {
        ...
        await this.firestore.updateDocumentValue(Db.usersCollName, `${id}`, { password: newP });
    }
    ...
    async deleteUser(token: string) {
        ...
        await this.firestore.deleteDocument(Db.usersCollName, `${user.id}`);
    ...
    async addDevice(deviceName: string, userAdminId: number, deviceKey?: string): Promise<number> {
        ...
        const maxId = await this.getMaxDeviceId(true);
    ...
    async changeDeviceAdmin(deviceId: number, userId: number) {
        ...
        await this.giveWriteDeviceRightsToUser(device.userAdminId, deviceId);
        await this.firestore.updateDocumentValue(Db.devCollName, `${deviceId}`, {
            userAdminId: userId,
        });
        await this.deleteUserRightForNewAdmin(userId, deviceId);
    }
    ...
    async sendEmailConfirmation_registration(id: number, username: String, email: string) {
        ...
        await this.firestore.setDocumentValue(Db.emailConfirmationsCollName, hashCode, emailConfirmationData);
    }
    ...
    async sendEmailConfirmation_addEmail(id: number, username: String, email: string) {
        ...
        let confirmations: IEmailConfirmationData[] = await this.firestore.getCollectionData(Db.emailConfirmationsCollName);
        for (let confirmation of confirmations) {
            if (confirmation.email === email) {
                await this.firestore.deleteDocument(Db.emailConfirmationsCollName, confirmation.hashCode);
            }
        }
        await this.firestore.setDocumentValue(Db.emailConfirmationsCollName, hashCode, emailConfirmationData);
    }
    ...
    async getEmailConfirmationData(hashCode: string): Promise<IEmailConfirmationData> {
        let data: IEmailConfirmationData[] = await this.firestore.getCollectionData(Db.emailConfirmationsCollName);
    ...
    async confirmEmail(hashCode: string) {
        ...
        if (findCode) {
            ...
            if (await this.getUserbyId(findCode.userId)) {
                await this.firestore.updateDocumentValue(Db.usersCollName, `${findCode.userId}`, { email: findCode.email });
                await this.firestore.deleteDocument(Db.emailConfirmationsCollName, findCode.hashCode);
    ...
    async getForgotPasswordRequest(hashCode: string): Promise<IForgotPasswordData> {
        let datas: IForgotPasswordData[] = await this.firestore.getCollectionData(Db.forgetPasswordRequestsCollName);
    ...
    async changePasswordViaForgetPasswordRequest(hashCode: string, newPassword: string) {
        let reqs: IForgotPasswordData[] = await this.firestore.getCollectionData(Db.forgetPasswordRequestsCollName);
        ...
        await this.firestore.updateDocumentValue(Db.usersCollName, `${req.userId}`, { password: newPassword });
        await this.firestore.deleteDocument(Db.forgetPasswordRequestsCollName, `${req.userId}`);
    }
